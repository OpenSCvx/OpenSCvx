...
def PTR_init(ocp: cp.Problem, discretization_solver: callable, params: Config):
    ...
    if 'x_init' in ocp.param_dict:
        ocp.param_dict['x_init'].value = params.sim.x.initial
    
    if 'x_term' in ocp.param_dict:
        ocp.param_dict['x_term'].value = params.sim.x.final

    # Solve a dumb problem to intilize DPP and JAX jacobians
    _ = PTR_subproblem(cpg_solve, params.sim.x, params.sim.u, discretization_solver, ocp, params)
...
def PTR_main(params: Config, prob: cp.Problem, aug_dy: callable, cpg_solve, emitter_function) -> dict:
    ...
    if 'x_init' in prob.param_dict:
        prob.param_dict['x_init'].value = params.sim.x.initial
    
    if 'x_term' in prob.param_dict:
        prob.param_dict['x_term'].value = params.sim.x.final
    ...
    while k <= params.scp.k_max and ((J_tr >= params.scp.ep_tr) or (J_vb >= params.scp.ep_vb) or (J_vc >= params.scp.ep_vc)):
        x_sol, u_sol, cost, J_total, J_vb_vec, J_vc_vec, J_tr_vec, prob_stat, V_multi_shoot, subprop_time, dis_time = PTR_subproblem(cpg_solve, x, u, aug_dy, prob, params)
    ...
    result = dict(
        ...
        t_final = x.guess[:,params.sim.idx_t][-1],
        u = u,
        x = x,
        x_history = scp_trajs,
...
def PTR_subproblem(cpg_solve, x, u, aug_dy, prob, params: Config):
    prob.param_dict['x_bar'].value = x.guess
    prob.param_dict['u_bar'].value = u.guess
    
    t0 = time.time()
    A_bar, B_bar, C_bar, z_bar, V_multi_shoot = aug_dy.call(x.guess, u.guess.astype(float))
    ...
    x_new_guess = (params.sim.S_x @ prob.var_dict['x'].value.T + np.expand_dims(params.sim.c_x, axis = 1)).T
    u_new_guess = (params.sim.S_u @ prob.var_dict['u'].value.T + np.expand_dims(params.sim.c_u, axis = 1)).T
