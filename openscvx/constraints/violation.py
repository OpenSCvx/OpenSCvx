...
def get_g_grad_x(
    constraints_ctcs: list[CTCSConstraint],
) -> Callable[[jnp.ndarray, jnp.ndarray, int], jnp.ndarray]:
    def g_grad_x(x: jnp.ndarray, u: jnp.ndarray, node: int) -> jnp.ndarray:
        grads = [c.grad_f_x(x, u, node) for c in constraints_ctcs if c.grad_f_x is not None]
...
def get_g_grad_u(
    ...
    def g_grad_u(x: jnp.ndarray, u: jnp.ndarray, node: int) -> jnp.ndarray:
        grads = [c.grad_f_u(x, u, node) for c in constraints_ctcs if c.grad_f_u is not None]
        return sum(grads) if grads else None
...
def get_g_funcs(constraints_ctcs: list[CTCSConstraint]) -> list[CTCSViolation]:
    ...
    for _idx, bucket in sorted(groups.items(), key=lambda kv: kv[0]):
        g = get_g_func(bucket)
