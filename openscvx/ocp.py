...
def OptimalControlProblem(settings: Config):
    ...
    constr += [x_nonscaled[i][settings.sim.idx_x_true] <= settings.sim.x.max[settings.sim.idx_x_true] for i in range(settings.scp.n)]
    constr += [x_nonscaled[i][settings.sim.idx_x_true] >= settings.sim.x.min[settings.sim.idx_x_true] for i in range(settings.scp.n)] # State Constraints (Also implemented in CTCS but included for numerical stability)
