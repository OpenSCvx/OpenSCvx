...
def simulate_nonlinear_time(x_0, u_lam, tau_vals, t, aug_dy, params: Config):
    ...
    for k in range(params.scp.n-1):
        ...
        sol = itg.solve_ivp(aug_dy.prop_aug_dy, (tau[k], tau[k+1]), x_0, args=(np.array(controls_current), np.array(controls_next), np.array([[tau[k]]]), params.sim.s_inds), method='DOP853', dense_output=True)
        x = sol.y
